...
function renderPrices () {
	for (let k of document.querySelectorAll("[data-price]")) {
		let price = Math.ceil(+k.getAttribute("data-price") * main.currency.list[main.currency.selected].rate * 100)/100;
		if (k.hasAttribute("data-price-fixed")) price = price.toFixed(+k.getAttribute("data-price-fixed"));
		k.innerHTML = `${(main.currency.list[main.currency.selected]?.prefix ?? true) ? (main.currency.list[main.currency.selected]?.symbol ?? "(error)") : ""}${Number.isNaN(+k.getAttribute("data-price")) ? "-" : price}${!(main.currency.list[main.currency.selected]?.prefix ?? true) ? main.currency.list[main.currency.selected].symbol : ""}`;
	}
}
