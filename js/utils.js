...
main.page.modal = function ({header, body, footer, options}) {
    ...
    if (footer.buttons.length > 0) {
    	...
    	for (let button of footer.buttons) html += `<button${typeof button.attributes === "object" && button.attributes.type === "string" ? "" : ` type="button"`} class="${typeof button.class === "string" ? button.class : "btn btn-secondary"}"${typeof button.close === "boolean" && button.close === false ? "" : ' data-bs-dismiss="modal"'}${typeof button.attributes === "object" ? (() => {let t="";for(let k in button.attributes)t+=` ${k}="${button.attributes[k]}"`;return t})() : ""}>${button.text}</button>`
    	html += `</div>`;
    }
	html += `</div></div></div>`;
